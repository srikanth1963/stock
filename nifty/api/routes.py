...
async def dashboard():
    ...
    system = {
        ...
        "day_bias": json.load(open("/opt/smb-algo/config/strategies.json"))["strategies"][0].get("day_bias", "range"),
    }
        ...
        trade_list = [
            {
                ...
                "signal": t.signal,
                "instrument": f"NIFTY {t.strike} {t.option_type}",
...
async def update_lot_size(request: Request):
    ...
    config_path = "/opt/smb-algo/config/strategies.json"
...
async def update_trading_mode(request: Request):
    ...
    # Update in-memory environment so change takes effect immediately without restart
    from dotenv import load_dotenv
    load_dotenv("/opt/smb-algo/.env", override=True)
...
async def update_day_bias(request: Request):
    ...
    import json as j
    cfg_path = "/opt/smb-algo/config/strategies.json"
