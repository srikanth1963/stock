...
from strategies.utlrg.expiry import get_expiry_date, get_expiry_string
...
async def get_spot_price(account: dict) -> Optional[float]:
    from core.breeze_client import get_spot
    for attempt in range(1, 4):
        spot = await get_spot(account, "NIFTY")
        if spot:
            return spot
        if attempt < 3:
...
async def live_order(account: dict, action: str, strike: int, option_type: str,
    ...
    for attempt in range(1, 3):
        ...
        order_id, fill_price = await place_limit_order(
            account=account, action=action, stock_code="NIFTY", strike=strike,
...
async def eod_squareoff(strategy_id: str):
    ...
    try:
        ...
        json.dump(data, open('/opt/smb-algo/config/strategies.json', 'w'), indent=2)
